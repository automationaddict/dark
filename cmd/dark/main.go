...
import (
	...
	dtsvc "github.com/automationaddict/dark/internal/services/datetime"
	inputsvc "github.com/automationaddict/dark/internal/services/input"
	keybindsvc "github.com/automationaddict/dark/internal/services/keybind"
	liminesvc "github.com/automationaddict/dark/internal/services/limine"
	appearancesvc "github.com/automationaddict/dark/internal/services/appearance"
	privacysvc "github.com/automationaddict/dark/internal/services/privacy"
	userssvc "github.com/automationaddict/dark/internal/services/users"
	notifycfgsvc "github.com/automationaddict/dark/internal/services/notifycfg"
...
func main() {
	...
	workspacesSub, err := nc.Subscribe(bus.SubjectWorkspacesSnapshot, func(m *nats.Msg) {
		var snap workspacessvc.Snapshot
		if err := json.Unmarshal(m.Data, &snap); err != nil {
			warnDecode("Workspaces", err)
			return
		}
		p.Send(tui.WorkspacesMsg(snap))
	})
	if err != nil {
		fmt.Fprintln(os.Stderr, "dark: subscribe workspaces:", err)
		os.Exit(1)
	}
	...
	if reply, err := nc.Request(bus.SubjectWorkspacesSnapshotCmd, nil, core.TimeoutFast); err == nil {
		var snap workspacessvc.Snapshot
		if err := json.Unmarshal(reply.Data, &snap); err == nil {
